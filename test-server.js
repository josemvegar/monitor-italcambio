...
app.post('/appointmentAPI/public/exchange/availaptmentbyhour.php', (req, res) => {
  ...
  if (testMode === 'no-availability') {
    ...
    console.log(`📡 [DISPO] Sin disponibilidad`);
  } 
  else if (testMode === 'availability' || testMode === 'booking-success' || testMode === 'booking-failed') {
    // En estos modos, SIEMPRE dar disponibilidad para poder probar el agendamiento
    responseData = [
    ...
    console.log(`🎯 [DISPO] Forzando disponibilidad (modo: ${testMode})`);
  }
    ...
    if (responseCount % 5 === 0) {
      ...
      responseData = { "message": "Sin Disponibilidad" };
      console.log(`📡 [DISPO] Sin disponibilidad (random)`);
    }
  }
  else {
    responseData = { "message": "Sin Disponibilidad" };
    console.log(`📡 [DISPO] Sin disponibilidad (default)`);
  }
...
app.get('/test-mode/:mode', (req, res) => {
  ...
  if (validModes.includes(mode)) {
    ...
    if (mode === 'booking-failed') {
      console.log('   • SIEMPRE habrá disponibilidad');
      console.log('   • PERO los agendamientos SIEMPRE fallarán');
    } else if (mode === 'booking-success') {
      console.log('   • SIEMPRE habrá disponibilidad');  
      console.log('   • Y los agendamientos SIEMPRE tendrán éxito');
    }
...
app.listen(PORT, () => {
  ...
  console.log('   http://localhost:3001/test-mode/booking-failed   - Siempre agendamiento fallido');
